  ...
  switch (name) {
    ...
    case 'fetch_url': {
      try {
        ...
        // Seul plafond technique optionnel (pas de clipping par défaut)
...
export default {
  async fetch(req, env, ctx) {
    ...
    if (url.pathname === '/') {
      return new Response(
        JSON.stringify({ ok: true, tools: ALL_TOOLS.map((t) => t.function.name) }),
    ...
    if (req.method === 'POST' && url.pathname === '/v1/chat/completions') {
      try {
        ...
        const model = body.model || env.OPENAI_MODEL || DEFAULT_MODEL;
        const finalMessage = await runAgent(body.messages, env, body);

        if (body.stream) {
          return new Response(buildSSEStream(finalMessage, model), {
            headers: {
        ...
        return new Response(JSON.stringify(buildJSONResponse(finalMessage, model)), {
          headers: { 'Content-Type': 'application/json', ...corsHeaders() }
        });
      } catch (e) {
